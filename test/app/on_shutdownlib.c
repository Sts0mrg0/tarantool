...
{
	fprintf(stderr, "on_shutdown_module_stop_func\n");
	fiber_wakeup(module.fiber);
	return 0;
}

/** Shutdown function to wait the main fiber of the module during shutdown */
static int
on_shutdown_module_join_func(void *arg)
{
	fprintf(stderr, "on_shutdown_module_join_func start\n");
	fiber_join(module.fiber);
	fprintf(stderr, "on_shutdown_module_join_func finished\n");
	return 0;
}
...
cfg(lua_State *L)
{
	...
	if (__atomic_load_n(&module.is_running, __ATOMIC_SEQ_CST) == 1)
		return 0;
	/*
	 * First we register the function that should be called last.
	 */
	fail_unless(box_on_shutdown(&module,
				    on_shutdown_module_join_func,
				    NULL) == 0);
	fail_unless(box_on_shutdown(&module,
